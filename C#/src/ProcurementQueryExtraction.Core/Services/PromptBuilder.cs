...
public static class PromptBuilder
{
    ...
    /// </summary>
    public const int DefaultRecentDays = 90;
    ...
    /// </summary>
    /// <param name="recentDays">Number of days to use for "recent" date queries. If null, uses DefaultRecentDays (90 days).</param>
    /// <returns>The system prompt string.</returns>
    public static string BuildSystemPrompt(int? recentDays = null)
    {
        ...
        var fiveYearsAgo = today.AddYears(-5).ToString("yyyy-MM-dd");
        var twoYearsAgo = today.AddYears(-2).ToString("yyyy-MM-dd");
        var oneYearAgo = today.AddYears(-1).ToString("yyyy-MM-dd");

        // Calculate "recent" date based on configurable days
        var recentDate = today.AddDays(-recentDays.Value).ToString("yyyy-MM-dd");

        // Calculate current and previous fiscal years
        // Federal FY starts Oct 1 of previous calendar year
        string currentFyStart, currentFyEnd, twoFyAgoStart;
        if (today.Month >= 10)
        {
            currentFyStart = new DateTime(today.Year, 10, 1).ToString("yyyy-MM-dd");
            currentFyEnd = new DateTime(today.Year + 1, 9, 30).ToString("yyyy-MM-dd");
            twoFyAgoStart = new DateTime(today.Year - 2, 10, 1).ToString("yyyy-MM-dd");
        }
        else
        {
            currentFyStart = new DateTime(today.Year - 1, 10, 1).ToString("yyyy-MM-dd");
            currentFyEnd = new DateTime(today.Year, 9, 30).ToString("yyyy-MM-dd");
            twoFyAgoStart = new DateTime(today.Year - 3, 10, 1).ToString("yyyy-MM-dd");
        }

        return $@"You are an expert in U.S. federal procurement data extraction. Your task is to convert natural language questions about federal procurement contracts into structured query filters.
...
Use SQL operators (=, <, >, <=, >=, BETWEEN) and calculate actual dates based on today's date ({todayStr}).

**RECENT DATE DEFINITION**: ""Recent"" means the last {recentDays} days (from {recentDate} to {todayStr}).
...
4. **AMBIGUOUS dates** (no clear start/end context): Populate BOTH fields with SAME values
   ...
   - ""within the last 5 years"" → BOTH fields = {{""operator"": ""BETWEEN"", ""start_date"": ""{fiveYearsAgo}"", ""end_date"": ""{todayStr}""}}

5. **""Recent"" keyword**: When user says ""recent"", ""recently"", or ""latest"", populate BOTH fields with recent_days
   - ""recent contracts"" → BOTH StartDate AND EndDate = {{""operator"": ""BETWEEN"", ""start_date"": ""{recentDate}"", ""end_date"": ""{todayStr}"", ""recent_days"": {recentDays}}}
...
**Relative dates**: Calculate from today ({todayStr})
- ""within the last 5 years"" → {{""operator"": ""BETWEEN"", ""start_date"": ""{fiveYearsAgo}"", ""end_date"": ""{todayStr}""}}
- ""within the last 2 years"" → {{""operator"": ""BETWEEN"", ""start_date"": ""{twoYearsAgo}"", ""end_date"": ""{todayStr}""}}
- ""past two fiscal years"" → {{""operator"": ""BETWEEN"", ""start_date"": ""{twoFyAgoStart}"", ""end_date"": ""{todayStr}""}}
...
- filter_groups: [
    {{
      ...
      ""StartDate"": {{""operator"": ""BETWEEN"", ""start_date"": ""{twoFyAgoStart}"", ""end_date"": ""{todayStr}""}},
      ""EndDate"": {{""operator"": ""BETWEEN"", ""start_date"": ""{twoFyAgoStart}"", ""end_date"": ""{todayStr}""}},
...
- filter_groups: [
    {{
      ...
      ""StartDate"": {{""operator"": ""BETWEEN"", ""start_date"": ""{fiveYearsAgo}"", ""end_date"": ""{todayStr}""}},
      ""EndDate"": {{""operator"": ""BETWEEN"", ""start_date"": ""{fiveYearsAgo}"", ""end_date"": ""{todayStr}""}},
...
- filter_groups: [
    {{
      ""subdoctype"": {{""operator"": ""="", ""value"": ""Contract""}},
      ""StartDate"": {{""operator"": ""BETWEEN"", ""start_date"": ""{recentDate}"", ""end_date"": ""{todayStr}"", ""recent_days"": {recentDays}}},
      ""EndDate"": {{""operator"": ""BETWEEN"", ""start_date"": ""{recentDate}"", ""end_date"": ""{todayStr}"", ""recent_days"": {recentDays}}},
...
Remember: Be precise, use SQL operators (=, >, <, >=, <=, BETWEEN, IN, LIKE), calculate actual dates, do NOT generate full PSC/NAICS codes - only extract what the user explicitly provides. When a description is provided, always identify the appropriate Level 1 and Level 2 categories from the lookup tables. Only include recent_days when the query uses ""recent"", ""recently"", or ""latest"". Always use filter_groups structure; set group_operator_between_groups to null for single group, ""OR"" for multiple groups with OR logic. For dates: use StartDate for period of performance start, EndDate for period of performance end. When ambiguous, populate BOTH with same values.";
    ...
    /// </summary>
    /// <param name="query">The natural language question about procurement contracts.</param>
    /// <returns>Formatted user prompt.</returns>
    public static string BuildUserPrompt(string query)
    {
        return $@"Extract structured query filters from the following natural language question about U.S. federal procurement contracts:
